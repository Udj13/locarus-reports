...
if (@$_POST['report']==1) {
    ...
    $hRes = $db->query("SELECT id_device, imei_name, black_date, number, model from main.devices $where");
    foreach($hRes->fetchAll() as $row) {
        $id = $row['id_device'];
		echo "<h2>#" . $id . " | " . $row['imei_name'] ." | " . $row['number'] ." | " . $row['model'] ." | " . $row['black_date'] . "</h2>\n";
        $where2 = (@$_POST['date']) ? "AND CAST(\"date\" as text) LIKE '" . mysql_escape_string($_POST['date']) . "%'" : '';
